(function($) {
  ...
  $('.has-submenu').on('mouseenter', function(){
  	$('.submenu-nav',this).addClass('active');
  })
  $('.has-submenu').on('mouseleave', function(){
  	$('.submenu-nav',this).removeClass('active');
  })
  ...
  var swiperTab = new Swiper('.category-shop-slider-container', {
    slidesPerView: 3,
    loop: true,
    spaceBetween : 6,
    navigation: {
...
function fancybox_destroy() {
    ...
    $(".lightbox-image").attr('target', '_blank');
}
  // Images Zoom
  $('.zoom-hover').zoom();
...
/* ==========================================================================
  ...
  $('.js-img-parallax .img-wrap').each(function (index, el) {
    var id = 'img-parallax-' + index;
    var $this = $(this);
    $this.attr('id', id);

    var offset = parseInt($this.attr('data-y'), 10) || 200;

    var tween = TweenMax.to('#' + id , 2, {
        y: offset
    });
...
$('a[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
  ...
  if ($thePane.find('.category-shop-slider-container').length > 0 && 0 === $thePane.find('.swiper-slide-active').length) {
    swiperTab[paneIndex].update();
  }
})
