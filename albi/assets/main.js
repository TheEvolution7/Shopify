...
(function ($) {
	...
	$('.box-custom-color').fadeOut()
	$('#color').on('change', function() {
		if ($(this).val() == 'custom' ) {
			$('.box-custom-color').fadeIn()
		}
		else {
			$('.box-custom-color').fadeOut()
		}
	...
	$('.colorContainer').click(function(e) {
		var $this = $(this);
		var color = e.target.style.background;
		$('.box-color').css('background', color)
		$('.colorContainer').removeClass('activeColor');
		$this.addClass('activeColor');
		$('#customcolor').val(color) 
	  });
	  
	  // Input change background after input of 3 or more hex values
	  $('.inputContainer input').keyup(function (){
		var $this = $(this).val().length;
		if ($this > 2) {
		  var $newHexValue = '#' + $(this).val();
		  $('.box-color').css('background', $newHexValue);
		  $('#customcolor').val($newHexValue)
		  console.log($('#customcolor').val($newHexValue))
		}
		$('#customcolor').val($newHexValue) 
	});
	...
	$(".qtybutton").on("click", function () {
		...
		var $button = $(this);
		var oldValue = $button.closest('.cart-plus').find("input.qty-input").val();
		...
		$button.closest('.cart-plus').find("input.qty-input").val(newVal);
	
	});


	$('#page-lost-password').hide();
        $('#move-to-lost').on('click',function() {
            $('#page-lost-password').show();
            $('#page-login').hide();
        })
